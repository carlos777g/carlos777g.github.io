...
const MarkdownComponents = {
  ...
  a({ href, children }) {
    ...
    if (isWikilink) {
      return (
        <span className="text-accent/60 font-mono text-sm">{children}</span>
      );
    }
    return (
      <a
  ...
  h1({ children }) {
    return (
      <h1 className="text-3xl md:text-4xl font-itim tracking-tighter mt-10 mb-4">
        {children}
      </h1>
    );
  },
  h2({ children }) {
    return (
      <h2 className="text-2xl md:text-3xl font-itim tracking-tighter mt-8 mb-3 text-accent">
        {children}
      </h2>
    );
  },
  h3({ children }) {
    return (
      <h3 className="text-xl font-bold mt-6 mb-2 text-glass-white">
        {children}
      </h3>
    );
  },
  p({ children }) {
    return (
      <p className="text-glass-white/80 leading-relaxed mb-4 text-sm md:text-base">
        {children}
      </p>
    );
  },
  ul({ children }) {
    return (
      <ul className="list-disc list-inside space-y-2 mb-4 text-glass-white/80 text-sm md:text-base">
        {children}
      </ul>
    );
  },
  ol({ children }) {
    return (
      <ol className="list-decimal list-inside space-y-2 mb-4 text-glass-white/80 text-sm md:text-base">
        {children}
      </ol>
    );
  },
          ...
          <ScrollReveal direction="up" duration={400}>
            ...
            >
              ...
              >
                <path
                  fill="currentColor"
                  d="M6.707 2.293a1 1 0 0 0-1.414 0l-4.5 4.5a1 1 0 0 0 0 1.414l4.5 4.5a1 1 0 0 0 1.414-1.414L3.414 8.5H13.5a1 1 0 0 0 0-2H3.414l3.293-3.293a1 1 0 0 0 0-1.414"
                />
              </svg>
              Back to blog
            </Link>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6 text-xs font-mono text-muted-white/50">
              ...
              {post.tags.length > 0 && (
                <div className="flex items-center gap-2">
                  <span className="text-glass-white/40 uppercase tracking-wider text-[10px]">
                    Tags:
                  </span>
                  <ul className="flex flex-wrap gap-2">
                    {post.tags.map((tag) => (
                      <li
                        key={tag}
                        className="px-2 py-0.5 bg-muted-white/10 text-glass-white/60 rounded-xl"
                      >
                        {tag}
                      </li>
                    ))}
              ...
              {post.tools.length > 0 && (
                <div className="flex items-center gap-2">
                  <span className="text-accent/40 uppercase tracking-wider text-[10px]">
                    Tools:
                  </span>
                  <ul className="flex flex-wrap gap-2">
                    {post.tools.map((tool) => (
                      <li
                        key={tool}
                        className="px-2 py-0.5 bg-accent/10 text-accent/70 rounded-xl"
                      >
                        {tool}
                      </li>
                    ))}
